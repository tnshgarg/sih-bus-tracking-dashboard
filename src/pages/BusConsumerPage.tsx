...
function generateRouteBusMap() {
  ...
  for (let i = 0; i < cityList.length; i++) {
    for (let j = 0; j < cityList.length; j++) {
      ...
      for (let k = 0; k < 7; k++) {
        const depHr = 15 + Math.floor(Math.random() * 9);
        const dep = `${String(depHr).padStart(2, "0")}:${k % 2 === 0 ? "00" : "30"}`;
        const arrHr = (depHr + Math.floor(Math.random() * 12)) % 24;
        const arr = `${String(arrHr).padStart(2, "0")}:${k % 2 === 0 ? "30" : "15"}`;
...
export default function BusConsumerPage() {
  ...
  let filteredBuses =
    !showResults || !pickup || !drop
      ? []
      : (routeBusMap[`${pickup}-${drop}`] || []).filter((bus) => {
          ...
          const depMinV = toDepartureMinutes(bus.departure);
          if (depMinV < depMin || depMinV > depMax) return false;

          const arr = toDepartureMinutes(bus.arrival);
          if (arr < arrivalMin || arr > arrivalMax) return false;

          return true;
        });

  // sorting (don't mutate original)
  let sortedBuses = filteredBuses;
  if (sortBy === "price-asc") sortedBuses = [...filteredBuses].sort((a, b) => a.price - b.price);
  else if (sortBy === "price-desc") sortedBuses = [...filteredBuses].sort((a, b) => b.price - a.price);
  else if (sortBy === "dep-asc") sortedBuses = [...filteredBuses].sort((a, b) => toDepartureMinutes(a.departure) - toDepartureMinutes(b.departure));
  else if (sortBy === "dep-desc") sortedBuses = [...filteredBuses].sort((a, b) => toDepartureMinutes(b.departure) - toDepartureMinutes(a.departure));
      ...
      <div className="flex-1 lg:pl-55">
        <div className="max-w-4xl mx-auto py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2 text-center mx-auto">
              Bus Portal
            </h1>
          </div>
            ...
            <div className="flex flex-col md:flex-row gap-5">
              ...
              <div className="flex-1">
                <CityAutocomplete
                  ...
                  value={pickup}
                  onChange={(v) => { setPickup(v); setShowResults(false); }}
                  cityList={cityList}
                />
              </div>
              ...
              <div className="flex-1">
                <CityAutocomplete
                  ...
                  value={drop}
                  disabled={!pickup}
                  onChange={(v) => { setDrop(v); setShowResults(false); }}
                  cityList={cityList.filter((c) => c !== pickup)}
                />
              </div>
                  ...
                  <div className="flex gap-2 items-center">
                    <select
                      ...
                      onChange={e => {
                        ...
                        else { /* default/custom -> do nothing */ }
                      }}
                    >
          ...
          {showResults && (
            <div>
              <h2 className="text-xl font-bold mb-4 text-gray-800">
                Buses from <span className="text-blue-600">{pickup}</span> to{" "}
                <span className="text-blue-600">{drop}</span>
              </h2>

              {sortedBuses.length === 0 ? (
                <div className="text-gray-500 mt-4 bg-white rounded-lg p-8 text-center border">
                  No buses found for this route with these filters.
                </div>
              ) : (
                <div className="space-y-5">
                  {sortedBuses.map((bus) => (
                    <div
                      key={bus.bus_id}
                      className="bg-white rounded-xl shadow-md border p-5 flex flex-col md:flex-row items-center justify-between"
                    >
                      <div className="flex-1">
