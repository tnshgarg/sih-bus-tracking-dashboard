...
const EditDriverModal = ({ isOpen, onClose, driver }: EditDriverModalProps) => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({
    name: driver?.name || "",
    licenseNo: driver?.license_no || "",
    phone: driver?.phone || "",
    status: driver?.status === "On Duty" ? "active" : "inactive",
    password: "",
  });
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      const response = await fetch(
        `${API_CONFIG.BASE_URL}${ENDPOINTS.ADMIN.FLEET_DRIVERS}/${driver.driver_id}`,
        {
          method: "PUT",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Edit Driver - {driver?.driver_id}</DialogTitle>
        </DialogHeader>
